      ...
      $.getJSON("ajax/get_material_availability.php", { material_id: matId })
        .done(function (data) {
          ...
          data.records.forEach(d => {
            const disabled = d.available === 0 ? "disabled" : "";
            html += `
                <button class="list-group-item list-group-item-action available-date"
                        data-date="${d.date}"
                        ${disabled}>
                    ${d.date} (${d.periode.replace("_", " ")}) – ${d.available} beschikbaar
                </button>`;
          });
          ...
          $(".available-date").on("click", function () {
            ...
            $.getJSON("ajax/get_material_availability.php", { material_id: matId, date: date })
              .done(function (info) {
                ...
                Object.keys(segments).forEach(periode => {
                  ...
                  html += `
                    <button type="button" class="btn btn-outline-primary w-100 select-periode-btn" data-periode="${periode}" data-available="${seg.available}" ${disabled}>
                      ${periode.replace("_", " ")} (${seg.available} beschikbaar)
                    </button>
                  `;
                });
                ...
                $(".select-periode-btn").on("click", function () {
                  ...
                  $("#availableText").html(`Beschikbaar in dit dagdeel: <strong>${available}</strong>`);
                });
              });
          });
